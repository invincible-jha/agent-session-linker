# Architecture — agent-session-linker

## Overview

Cross-session context persistence and resumption

This document describes the high-level architecture of agent-session-linker
...
agent-session-linker uses a decorator-based plugin registry backed by
``importlib.metadata`` entry-points. This allows third-party packages
(including the AgentContext enterprise edition) to extend the system
...
from agent_session_linker.core import BaseProcessor  # example base class
